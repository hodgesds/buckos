...
`sideros-web` provides the official Sideros website, including project documentation, installation guides, and wiki content. It's built with Axum and Askama for fast, type-safe web serving.

## Features

- **Project Homepage**: Overview and introduction to Sideros
...
```toml
[dependencies]
sideros-web = { path = "../web" }
```

Or install the binary:

```bash
cargo install --path sideros/web
```
...
```bash
cargo build --release
./target/release/sideros-web
```

### With Custom Port

```bash
SIDEROS_WEB_PORT=8080 sideros-web
```
...
|----------|---------|-------------|
| `SIDEROS_WEB_PORT` | `3000` | Server port |
| `SIDEROS_WEB_HOST` | `0.0.0.0` | Bind address |
| `RUST_LOG` | `info` | Log level |

### Configuration File

```toml
# /etc/sideros/web.toml
...
[content]
wiki_path = "/var/lib/sideros/wiki"
```

## API Endpoints

While primarily a website, `sideros-web` also provides some API endpoints:
...
COPY --from=builder /app/target/release/sideros-web /usr/local/bin/
...
```bash
# Run tests
cargo test -p sideros-web

# Test with coverage
cargo tarpaulin -p sideros-web

# Integration tests
cargo test -p sideros-web --features integration
```
...
- [Sideros Project](https://github.com/hodgesds/sideros)
