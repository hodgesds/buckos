...
Help and assistance utilities for Sideros users and developers.

## Overview

`sideros-assist` provides interactive help, troubleshooting guides, and assistance features for the Sideros operating system. It serves as the primary interface for users seeking help with system configuration, package management, and common issues.
...
```toml
[dependencies]
sideros-assist = { path = "../assist" }
```

Or install the binary:

```bash
cargo install --path sideros/assist
```
...
```rust
use sideros_assist::help::HelpSystem;
...
```rust
use sideros_assist::diagnostics::Diagnostics;
...
```rust
use sideros_assist::wizard::ConfigWizard;
...
```bash
# Run tests
cargo test -p sideros-assist

# Run with test documentation
ASSIST_DOCS=/path/to/test/docs cargo test -p sideros-assist
```
...
- [Sideros Documentation](https://sideros.dev/docs)
