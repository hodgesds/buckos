...
impl ConfigLoader {
    ...
    pub fn user() -> Result<Self> {
        ...
        Ok(Self::new(PathBuf::from(home).join(".config/sideros")))
    }
...
pub mod paths {
    ...
    pub fn package_db() -> PathBuf {
        PathBuf::from("/var/db/sideros")
    }

    /// Cache directory
    pub fn cache() -> PathBuf {
        PathBuf::from("/var/cache/sideros")
    }
    ...
    pub fn user_config() -> Option<PathBuf> {
        std::env::var("HOME")
            .ok()
            .map(|h| PathBuf::from(h).join(".config/sideros"))
    }
}
...
pub mod env_vars {
    ...
    pub const CONFIG_ROOT: &str = "SIDEROS_CONFIG_ROOT";
    /// Repository root override
    pub const REPO_ROOT: &str = "SIDEROS_REPO_ROOT";
    /// Cache directory override
    pub const CACHE_DIR: &str = "SIDEROS_CACHE_DIR";
...
mod tests {
    ...
    #[test]
    fn test_paths() {
        assert_eq!(paths::system_config(), PathBuf::from("/etc/sideros"));
        assert_eq!(paths::package_db(), PathBuf::from("/var/db/sideros"));
    }
