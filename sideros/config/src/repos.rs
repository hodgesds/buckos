...
impl ReposConfig {
    ...
    /// Create default Sideros repository configuration
    pub fn default_sideros() -> Self {
        let mut config = Self::new();

        config.defaults = RepoDefaults {
            main_repo: Some("sideros".to_string()),
            auto_sync: true,
        ...
        config.add_repo(Repository {
            name: "sideros".to_string(),
            location: PathBuf::from("/var/db/repos/sideros"),
            sync_type: SyncType::Git,
            sync_uri: Some("https://github.com/sideros/packages.git".to_string()),
            priority: 1000,
            auto_sync: true,
            clone_depth: Some(1),
...
mod tests {
    ...
    #[test]
    fn test_default_sideros() {
        let config = ReposConfig::default_sideros();
        assert!(config.has_repo("sideros"));

        let repo = config.get_repo("sideros").unwrap();
