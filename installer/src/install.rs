...
impl UseFlags {
    ...
    /// Generate Buck2 modifier Starlark for config/local_modifiers.bzl
    ///
    /// Produces a list of constraint_value targets that the root PACKAGE file
    /// loads and applies via set_cfg_modifiers(). This integrates with buckos-build's
    /// native Buck2 modifier/constraint system.
    pub fn to_modifier_bzl(&self) -> String {
        let mut modifiers = Vec::new();

        // Global USE flags → constraint targets
        for flag in &self.global {
            modifiers.push(format!("    \"//use/constraints:{}-on\",", flag));
        }

        // VIDEO_CARDS USE_EXPAND → use_expand_multi constraint targets
        for card in &self.video_cards {
            modifiers.push(format!(
                "    \"//use/constraints:video_cards_{}-on\",",
                card
            ));
        }

        // INPUT_DEVICES USE_EXPAND → use_expand_multi constraint targets
        for device in &self.input_devices {
            modifiers.push(format!(
                "    \"//use/constraints:input_devices_{}-on\",",
                device
            ));
        }
        ...
        format!(
            r#"# Auto-generated Buck2 modifier configuration
# Generated by BuckOS installer. Edit via: buckos use [flags]
...
"#
    ...
    /// Generate per-package PACKAGE file content using `set_cfg_modifiers()`.
    ///
    /// This is the proper Buck2 primitive for per-directory modifier overrides.
    pub fn generate_package_modifiers(flags: &[String]) -> String {
        let modifiers_str = flags
            .iter()
        ...
        format!(
            r#"# Auto-generated per-package USE override
# Generated by BuckOS installer. Edit via: buckos use <package> [flags]
...
"#
    ...
    /// Write per-package PACKAGE override files into the buckos-build repo
    pub fn write_package_overrides(
        &self,
        buckos_build_path: &std::path::Path,
    ) -> anyhow::Result<()> {
        use anyhow::Context;
        for (pkg, flags) in &self.package_use {
            // Find the package directory in the repo
            // Package names like "dracut" need to be resolved to their full path
            // For now, search for the package directory
            let packages_dir = buckos_build_path.join("packages");
            if let Some(pkg_dir) = find_package_dir(&packages_dir, pkg) {
                let package_file = pkg_dir.join("PACKAGE");
                let content = Self::generate_package_modifiers(flags);
                std::fs::write(&package_file, content)
                    .with_context(|| format!("Failed to write PACKAGE for {}", pkg))?;
                tracing::info!("Wrote per-package PACKAGE override for {}", pkg);
            } else {
                tracing::warn!(
...
///
/// Searches recursively for a directory matching the package name that
/// contains a BUCK file (indicating it's a valid package target).
fn find_package_dir(packages_dir: &std::path::Path, name: &str) -> Option<std::path::PathBuf> {
    if !packages_dir.exists() {
        return None;
    }
    // Walk the packages directory looking for a matching leaf directory with a BUCK file
    find_package_dir_recursive(packages_dir, name)
}

fn find_package_dir_recursive(dir: &std::path::Path, name: &str) -> Option<std::path::PathBuf> {
...
pub fn run_installation(config: InstallConfig, progress: Arc<Mutex<InstallProgress>>) {
    ...
    let run_step = || -> Result<()> {
        ...
        // Write Buck2 modifier config for buckos-build
        std::fs::write(&modifiers_path, use_flags.to_modifier_bzl())?;
        tracing::info!(
            "Generated local_modifiers.bzl with {} global flags, {} video cards, {} input devices",
            use_flags.global.len(),
            use_flags.video_cards.len(),
            use_flags.input_devices.len()
        );

        // Write per-package PACKAGE override files
        use_flags.write_package_overrides(&config.buckos_build_path)?;
