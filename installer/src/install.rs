...
impl UseFlags {
    ...
    /// Generate Buck2 modifier Starlark for config/install_modifiers.bzl
    ///
    /// Produces a list of constraint_value targets that the root PACKAGE file
    /// loads and applies via set_modifiers(). This integrates with buckos-build's
...
"#
    ...
    /// Generate per-package PACKAGE file content for a set of modifier overrides
...
pub fn run_installation(config: InstallConfig, progress: Arc<Mutex<InstallProgress>>) {
    ...
    let run_step = || -> Result<()> {
        ...
        tracing::info!(
            "Generated install_modifiers.bzl with {} global flags, {} video cards, {} input devices",
